...
public class AuthorizationCodeGrantHandler implements OAuth2GrantHandler {
	...
	public ApplyTokenResponse handle(ApplyTokenRequest request) {
		...
		// 未获取到授权码信息
		ErrorCode.CODE_NOT_FOUND.notNull(codeDomain);
		// 客户端不匹配
		ErrorCode.CODE_CLIENT_MISMATCH.isTrue(StrUtil.equals(codeDomain.clientId(), request.clientId()));
		// 重定向 URI 不匹配
		ErrorCode.CODE_REDIRECT_URI_MISMATCH.isTrue(StrUtil.equals(codeDomain.redirectUri(), request.redirectUri()));
		// 授权码被使用
		ErrorCode.CODE_USED.isFalse(codeDomain.used());
		// 授权码已过期
		ErrorCode.CODE_EXPIRED.isFalse(codeDomain.expiresTime().isBefore(LocalDateTime.now()));
		// 验证 codeVerifier
		boolean validateSuccess = codeDomain.validatePkce(request.codeVerifier());
		ErrorCode.PKCE_VERIFIER_FAILED.isTrue(validateSuccess);
		// TODO: 生成TOKEN
